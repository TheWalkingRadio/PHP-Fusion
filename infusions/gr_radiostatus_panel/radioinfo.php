...
if (isset($_GET['titlelink']) AND file_exists('cache/stream_data.php')) {
	...
	if (array_key_exists($_GET['titlelink'],$stream_data)) {
		if (file_exists('cache/stream_'.$stream_data[$_GET['titlelink']]['id'].'_'.$stream_data[$_GET['titlelink']]['cache'].'.php')) {
			...
			if ($cache['status'] == 1) {
				if ($stream_data[$_GET['titlelink']]['usertyp'] == 1) {
					echo $cache['aim'];
				} elseif ($stream_data[$_GET['titlelink']]['usertyp'] == 2) {
					echo $cache['icq'];
				} elseif ($stream_data[$_GET['titlelink']]['usertyp'] == 3) {
					echo $cache['irc'];
				} elseif ($stream_data[$_GET['titlelink']]['usertyp'] == 4) {
					echo $cache['server_title'];
				} elseif ($stream_data[$_GET['titlelink']]['usertyp'] == 5) {
					echo $cache['genre'];
				} else {
					echo $cache['aim'];
				}
